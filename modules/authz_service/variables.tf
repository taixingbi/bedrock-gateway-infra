...
variable "provisioned_principal_mappings_table_arn" {
  description = "Read-only: the same onboarding-provisioned principal-mappings table bedrock-gateway-app's task role writes to (modules/ecs_service). This service only reads it."
  type        = string
}
